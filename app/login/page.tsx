...
function LoginForm() {
  ...
  const callbackUrl = params.get("callbackUrl") || "/shorts";
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-16">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        <h1 className="text-2xl font-bold text-white">Entra a tu cuenta</h1>
        <p className="mt-1 text-sm text-white/60">Genera Reels con IA. 5 créditos gratis al registrarte.</p>
        ...
        <p className="mt-4 text-center text-sm text-white/60">
          ¿No tienes cuenta?{" "}
          <Link href="/register" className="text-violet-300 hover:text-violet-200 underline">
            Crear una gratis
          </Link>
        </p>
      </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="mx-auto max-w-md px-4 py-16 text-white/50">Cargando…</div>}>
      <LoginForm />
    </Suspense>
  );
}
